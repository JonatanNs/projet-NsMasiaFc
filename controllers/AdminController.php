...
class AdminController extends AbstractController{
    ...
    public function checkAddMatchs(){
        var_dump($_POST);
        if(isset($_POST["location"]) && isset($_POST["rivalTeam"]) && isset($_POST["date"])){
            $tokenManager = new CSRFTokenManager(); 
            if(isset($_POST["csrf-token"]) && $tokenManager->validateCSRFToken($_POST["csrf-token"])){
                ...
                $team = $rivalTeamManager->getAllRivalTeamsByName($_POST["rivalTeam"]);
                //var_dump($team->getId());
                $matchManager->createMatch($nsMasia[0]['id'], $team->getId(), $location, $formatted_date);
